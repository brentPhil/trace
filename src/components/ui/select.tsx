...
/**
 * A WRAPPER, NOT A RE-EXPORT, and the difference is a null that ten call sites
 * were each handling.
 *
 * Base UI types `onValueChange` as `(value: T | null, …)` because its Root can
 * represent "nothing selected". Passed straight through, every consumer in this
 * product wrote the same two lines —
 *
 *     onValueChange={(next) => { if (next !== null) setSort(next) }}
 *
 * — for a case none of them can reach: not one Select here is clearable, they
 * all mount with a value, and Base UI only produces `null` from an explicit
 * clear. Ten identical guards against an impossible value is what a correction
 * that stopped one component short looks like, so the null is swallowed once,
 * here, and `onValueChange` hands consumers a `TValue`.
 *
 * If a clearable Select is ever needed it takes `SelectPrimitive.Root` directly
 * and handles its own null — the honest shape for the component that has the
 * case.
 */
...
/** `rounded-md`, corrected from the registry's `rounded-3xl` — the same field
 *  radius every hand-rolled input in the app uses; see `ui/input.tsx` for the
 *  argument. The POPUP below stays on the 3xl step: it is a floating surface,
 *  not a field. */
