...
/*
 * /music — where a person manages the audio they uploaded.
 *
 * THE COMPONENT IS EXPORTED FROM THE ROUTE FILE, which is not how the other
 * seven pages here are built any more: each of those is a `-page.tsx` beside
 * its route, because an export of a route file is something TanStack Router's
 * code-splitter refuses to split. This one keeps both in one file because
 * Task 11's brief pins `export function Music()` in `src/routes/_authed/music.tsx`
 * and names that file, its test, and app-sidebar.tsx as the whole writable set
 * for the task — creating an eighth `-music.tsx` would have been outside it.
 * The split is a mechanical follow-up: move everything below `Route` into
 * `./-music`, import `Music` from there, and nothing else changes.
 *
 ...
 *
 * "RECENT" IS AN APPROXIMATION AND IS WORTH SAYING SO. `listTracks` returns
 * `_id` but not `_creationTime`, so the newest track cannot be identified
 * exactly; `_id` descending is the closest stand-in the returned shape allows.
 * The real fix is one line in convex/music.ts's `trackReturns` — add
 * `_creationTime` and sort on it — which is a change to Task 3's file and so
 * not made here.
 */
function orderTracks(
  ...
  filtered.sort((a, b) =>
    sort === "name" ? a.name.localeCompare(b.name) : b._id.localeCompare(a._id)
  )
  return filtered
}
