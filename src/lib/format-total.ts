 ...
 *   ONLY — never to a single entry's own row.
 *
 ...
 * on the same screen.
 *
 * A single row is excluded deliberately: "1.75" reads as a quantity where
 * "1h 45m" reads as a span someone spent, and a row is one stretch of work
 * rather than a figure to be added up. An unspecified decimal conversion on a
 * number beside an invoice is exactly the ambiguity the duration parser exists
 * to remove, so it is specified here.
 */
